...
import com.example.authentication.data.login.LoginErrorModel
...
sealed class LoginResult {
    ...
    data class Error(val errorModel: LoginErrorModel) : LoginResult()
}
...
class LoginModel {
    ...
    suspend fun login(email: String, password: String): LoginResult {
         return  try {
             ...
             val errorModel = parseError(e)
                LoginResult.Error(errorModel = errorModel)
            } catch (e: IOException) {
    ...
    private suspend fun parseError(exception: HttpException): LoginErrorModel {
        return withContext(Dispatchers.IO) {
            val errorBody = exception.response()?.errorBody()?.string()
            val gson = Gson()
            gson.fromJson(errorBody, LoginErrorModel::class.java)
        }
    }
}